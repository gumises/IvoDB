...
public class ButtonsPanel extends JPanel {
    ...
    public ButtonsPanel() {
        ...
        // make Backup
        tempButton = new ActionButton(MAKE_BACKUP, ADMIN, OTHER);
        tempButton.addActionListener(event -> buttonPressed(MAKE_BACKUP));
        actionButtons.add(tempButton);
    ...
    public void changeAccess(int access) {
        ...
        if (access == 2) {
            actionButtons.get(actionButtons.size() - 1).setText("Zaloguj");
        } else {
			actionButtons.get(actionButtons.size() - 1).setText("Wyloguj");
        }
    }
